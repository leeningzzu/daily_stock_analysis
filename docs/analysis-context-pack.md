...
- `daily_bars` 只表达完整日线窗口，优先读 `base_context.today`、`base_context.yesterday`、`base_context.date`、`base_context.data_missing`；date-only 放入 `value` 或 `metadata`，不写入 `timestamp`。MUE V1 在同一 block metadata 内新增低敏 `evidence_identity`：复用传入的 `MarketPhaseContext` 的 `market / phase / session_date / effective_daily_bar_date / is_partial_bar` 与 stored daily bar 的 `date / data_source / OHLCV / 已存 MA` 白名单字段生成稳定 `daily_bar_identity_sha256`。只有 `base_context.today.date == effective_daily_bar_date`，且现有 `resolve_historical_daily_bar_date()` 能从同一 market/session/phase fail-closed 证明该 effective date 时，`identity_state=READY`、`completed_bar_proven=true`；bar 更旧为 `STALE`，bar 晚于权威 completed date 为 `PARTIAL`，bar/source 缺失为 `MISSING`，phase/calendar/date 无法证明为 `UNKNOWN`。Identity 状态本身始终 fail-closed，但为保持既有 `AnalysisContextPack` consumer 向后兼容，缺少新增 identity 证明元数据不会追溯性把原本可用的 legacy `daily_bars` / DataQuality 改写为 missing；只有 identity 正向证明 `STALE` 或 `PARTIAL` 时才覆盖既有 block status，原始日线本身缺失仍沿用既有 `MISSING`。`is_partial_bar=true` 只表示当前交易日可能仍未完成，不会否决上一根已证明的完整日线。该 identity 只保留在 `daily_bars.metadata.evidence_identity` 这一单一 authority，供后续确定性 consumer 显式检查 READY；不新增 DB/schema version/fetcher，也不修复 `storage.get_analysis_context(target_date)` 当前仍读取最新两天的历史语义。
